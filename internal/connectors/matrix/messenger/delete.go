...
func (messenger *service) deleteMessage(deleteAction *Delete, retries uint, retryTime time.Duration) error {
	...
	for retries > 0 {
		...
		if err == nil {
			...
			messenger.logger.Infof("Deleting message is stopped since we ran in a rate limit")
			continue
	...
	messenger.logger.Infof("Deleting message failed and retries are exceeded. Error is: " + err.Error())
	return err
}
